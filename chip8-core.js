...
// Standard 8x5 font set for the hex digits 0-F. Each byte is one row,
...
Chip8.prototype.run = function (opcode) {
    ...
    switch (op) {
        case 0x0:
            ...
            if (opcode == 0x00ee) {
                // A return with an empty stack has no valid return address
                // (a buggy ROM). Report it and fall through to the next
                // opcode instead of popping a stale slot.
                if (this.sp === 0)
                    return "!!! 00EE with an empty stack, ignoring !!!";
        ...
        case 0xf:
            switch (N) {
                ...
                // Each glyph is 5 bytes, so the font spans 0x50-0x9F.
                case 0x29:
